...
class MusicServiceConnection(context: Context) {
    ...
    private inner class MediaBrowserConnectionCallback(private val context: Context):MediaBrowserCompat.ConnectionCallback() {
        ...
        override fun onConnectionSuspended() {
            _isConnected.postValue(Event(Resource.error("The connection was suspened", false)))
        }
    ...
    private inner class MediaControllerCallback : MediaControllerCompat.Callback() {
        ...
        override fun onSessionEvent(event: String?, extras: Bundle?) {
            ...
            when(event) {
                NETWORK_ERROR -> _netWorkError.postValue(Event(Resource.error("Couldn't connect",null)))
            }
        }
